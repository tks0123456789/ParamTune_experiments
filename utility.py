...
# }
# trials = Trials()
# eval_fn = make_eval_XGB_C(X_train, y_train, X_valid, y_valid,
#                           subsample=.8, n_folds=5, seed=321)
# best = fmin(eval_fn, parameter_space_xgb, algo=tpe.suggest, trials=trials, max_evals=200)

# For hyperopt
# I used make_.._old in exp001
def make_eval_XGB_C(X_train, y_train, X_valid, y_valid,
    ...
    def eval_fn(params):
        model = XGBClassifier(n_estimators=n_estimators_max, subsample=subsample,
...
def param_tune_experiment(param_data, param_space_xgb, max_evals=10, n_folds=5,
    ...
    eval_fn = make_eval_XGBC(X_train, y_train, X_valid=X_valid, y_valid=y_valid,
...
## old
def make_eval_XGB_C_old(X_train, y_train, X_valid, y_valid,
                    n_estimators_max=500, learning_rate=0.1,
                    n_folds=5, subsample=.8, seed=123, verbose=False):
    #n_estimators_lst = []
    #score_valid = []
